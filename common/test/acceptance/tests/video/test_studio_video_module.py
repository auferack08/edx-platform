        ...
        self.course_fixture.add_children(
            XBlockFixtureDesc('chapter', 'Test Section').add_children(
                XBlockFixtureDesc('sequential', 'Test Subsection').add_children(
                    XBlockFixtureDesc('vertical', 'Test Unit', publish='make_private').add_children(
...
class CMSVideoTest(CMSVideoBaseTest):
    ...
    def _create_video(self):
        ...
        xblocks = self.video.xblocks_count()
        types = self.video.xblock_types()
        ...
        self.assertTrue(xblocks == 2 and types[1] == 'video')
