...
class VideoComponentPage(VideoPage):
    ...
    def xblocks_count(self):
        """
        Get xblock component count on current page.

        Returns:
            (int): total xblock components

        """
        component_selector = '.component'
        return len(self.q(css=component_selector).results)

    def xblock_types(self):
        """
        Get types of all xblock components on current page.

        Returns:
            (list): xblock component types

        """
        xblock_component_type_selector = 'li.component .xblock'
        return self.q(css=xblock_component_type_selector).attrs('data-block-type')
